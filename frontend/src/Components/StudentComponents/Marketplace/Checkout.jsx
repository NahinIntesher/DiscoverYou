      ...
      if (checkoutResponse.data.status === "Success") {
        ...
        const notificationPromises = formData.products.map((product) =>
          axios.post("http://localhost:3000/student/notifications", {
            ...
            notificationTitle: "Product Checkout",
            notificationMessage: `${user.student_name} checked out your product "${product.productName}" in Marketplace!`,
            notificationLink: `/marketplace/product/${product.productId}`,
          })
        );
