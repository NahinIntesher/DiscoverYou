...
module.exports = (router, multer) => {
  ...
  router.post("/marketplace/add-to-cart", verifyToken, (req, res) => {
    ...
    connection.query(
      "SELECT * FROM marketplace_cart WHERE product_id = ? AND buyer_admin_id = ?",
      [productId, userId],
      (err, results) => {
        if (err) throw err;
        if (results.length > 0) {
          ...
          connection.query(
            "INSERT INTO marketplace_cart(product_id, buyer_admin_id) VALUES (?, ?);",
            [productId, userId],
  ...
  router.post("/marketplace/remove-from-cart", verifyToken, (req, res) => {
    ...
    connection.query(
      "DELETE FROM marketplace_cart WHERE product_id = ? AND buyer_admin_id = ?",
      [productId, userId],
      (err, results) => {
        if (err) throw err; 
        return res.json({ status: "Success" });
