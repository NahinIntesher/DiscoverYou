...
module.exports = (router) => {
  ...
  router.get("/notifications", verifyToken, (req, res) => {
    ...
    connection.query(
      "SELECT *, TIMESTAMPDIFF(SECOND, sent_time, NOW()) AS notification_time_ago FROM notifications WHERE recipient_admin_id = ? ORDER BY notification_id DESC",
      [userId],
      (err, results) => {
        if (err) {
          console.error("Error fetching notifications:", err);
  ...
  router.get("/notifications/new", verifyToken, (req, res) => {
    ...
    SELECT COUNT(*) AS new_notifications
      FROM notifications 
      WHERE recipient_admin_id = ? AND is_seen = 0;`;
